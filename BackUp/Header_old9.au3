...
#comments-start
	...
	- $bFlag: 0 or 1 -
	Boolean flag that determines what to do about CMW when ending the script.
	Flag Options:
...
Func _EndProg($bFlag = 0)
	Sleep(2500); pause a few seconds
...
Func _OpenApp($sApp)
	...
	if $bFlag Then
		If Not StringInStr(WinGetText($main), $wAppWindow) Then
			$bFlag = False
		EndIf
	EndIf
