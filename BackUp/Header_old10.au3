...
Func _OpenApp($sApp)
	...
	If $bFlag Then
		If Not StringInStr(WinGetText($g_wMain), $wAppWindow) Then
			$bFlag = False
		EndIf
	EndIf
	...
	Else
		ControlClick("[CLASS:TfrmMain_CMW]", "", $sAppButton)
...
Func TermTextWait($hTitle, $hText, $hControlID, $sTextToSearch, $tTimeout = 0)
	;	Sleep(100)
	;	If Not WinExists($hTitle, $hText) Then
	;		;SetError(1)
	;		MsgBox(0, "Error", "Could not find CMW window")
	;		Return -1
	;	EndIf
...
#comments-end
Func CaptureScreen($hTitle = $g_wMain, $sName = Null, $fiAltDir = Null)
